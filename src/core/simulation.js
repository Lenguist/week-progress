import { state, computeBreakdown, clamp } from './state.js';
...
export function resetAll() {
  state.week = 0; state.progress = 0; state.score = 0;
  const logEl = $('#log'); if (logEl) logEl.innerHTML = '';
  hideDice();
  const b = computeBreakdown(); updateProgress(b); drawFunnel(b); log('Simulation reset.');
}
